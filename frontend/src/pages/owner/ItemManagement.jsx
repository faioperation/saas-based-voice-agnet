    ...
    <div>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div className="flex-1 w-full max-w-xs">
          <Dropdown
            ...
            inputClass="!bg-sky-50 !border-[#272727] !text-sky-950 !rounded-xl !py-3.5 !px-4 !text-sm placeholder:!text-sky-950 !placeholder-white"
            optionClass="!bg-sky-50 !text-sky-950 !border-[#272727]"
            icon="!text-sky-700"
          />
        </div>
      ...
      <div className="w-full relative z-0">
        {items.length > 0 ? (
          <Table
            TableHeads={columns}
            TableRows={items}
            headClass=" border-b border-[#1A1A1A] text-gray-900 whitespace-nowrap last:[&>div]:justify-center"
      ...
      {isUploadModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-white/60 backdrop-blur-sm p-4">
          <div className="bg-white border border-gray-800 rounded-[20px] w-full max-w-[600px] p-8 relative shadow-2xl">
            <button
              onClick={() => {
                setIsUploadModalOpen(false);
                setUploadAgentId("");
            ...
            <h2 className="text-sky-950 text-xl font-bold mb-6">
              Upload Menu File
            </h2>

            <div className="mb-6">
              <label className="block text-sky-700 text-sm mb-2">
                Select Agent
              </label>
              <Dropdown
                ...
                inputClass="!bg-sky-50 !border-[#272727] !text-sky-950 !rounded-xl !py-3.5 !px-4 !text-sm w-full placeholder:!text-sky-950 !placeholder-white"
                optionClass="!bg-sky-50 !text-sky-950 !border-[#272727]"
                icon="!text-sky-700"
              />
            </div>
            ...
            <div className="mb-6">
              ...
              <div className="border-2 border-dashed border-gray-700 hover:border-gray-500 rounded-xl p-8 text-center transition-colors relative cursor-pointer group">
                ...
                <div className="flex flex-col items-center justify-center gap-3">
                  ...
                  <div>
                    <p className="text-sky-800 font-medium mb-1">
              ...
              {selectedFiles.length > 0 && (
                <div className="mt-4 space-y-2 max-h-[150px] overflow-y-auto pr-1">
                  {selectedFiles.map((file, idx) => (
                    <div
                      key={idx}
                      className="flex items-center justify-between bg-sky-50 border border-gray-800 rounded-lg p-3"
                    >
                      <div className="flex items-center gap-3 overflow-hidden">
                        <FileText className="w-5 h-5 text-sky-700 shrink-0" />
                        <span className="text-sky-800 text-sm truncate">
                          {file.name}
                        </span>
                      </div>
                      <button
                        onClick={() => removeFile(idx)}
                        className="text-sky-600 hover:text-red-500 transition-colors p-1 cursor-pointer"
                      >
                        <X className="w-4 h-4" />
                      </button>
                    </div>
            ...
            <div className="flex justify-end gap-3 mt-8">
              <button
                ...
                className="px-6 py-2.5 rounded-full text-sm font-semibold text-sky-950 bg-sky-50 border border-gray-800 hover:bg-gray-800 transition-colors cursor-pointer"
              >
                Cancel
              </button>
              <button
                ...
                className="flex items-center gap-2 bg-blue-600 text-sky-950 px-6 py-2.5 rounded-full text-sm font-semibold hover:bg-blue-700 transition-colors disabled:opacity-50 cursor-pointer"
              >
      ...
      {isDeleteModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-white/60 backdrop-blur-sm p-4">
          <div className="bg-white border border-gray-800 rounded-[20px] w-full max-w-[500px] p-8 relative shadow-2xl">
            ...
            <p className="text-sky-700 text-[14px] leading-relaxed mb-10 text-center px-4">
            ...
            <div className="flex items-center justify-center gap-5">
              <button
                onClick={() => setIsDeleteModalOpen(false)}
                className="bg-white text-white font-semibold px-10 py-2.5 rounded-full hover:bg-gray-200 transition-colors text-sm cursor-pointer"
              >
                Cancel
              </button>
              <button
                onClick={handleConfirmDelete}
                className="bg-red-500 text-sky-950 px-10 py-2.5 rounded-full font-semibold hover:bg-red-600 transition-colors text-sm cursor-pointer"
              >
                Delete
              </button>
            </div>
