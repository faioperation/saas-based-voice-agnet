...
const ViewTenant = () => {
    ...
    },
    {
      ...
      render: (row) => (
        <div className="text-left">
          <span className="w-[85px] inline-block text-center px-2 py-1 text-[11px] font-medium text-sky-950 rounded-[4px] bg-[#4285F4] capitalize">
            {row.status}
          </span>
        </div>
      ),
  ...
  const agentColumns = [
    {
      ...
      sortable: true,
      render: (row) => (
        <div className="text-left text-gray-900">{row.name || "Unknown"}</div>
      ),
    },
    {
      key: "vapi_id",
  ...
  const callColumns = [
    ...
    },
    {
      ...
      width: "20%",
      render: (row) => (
        <div className="text-left text-gray-900">{row.duration || "N/A"}</div>
      ),
    },
    {
      key: "time",
      ...
      width: "20%",
      render: (row) => (
        <div className="text-left text-gray-900">{row.time || "N/A"}</div>
      ),
    },
    {
      key: "date",
      ...
      render: (row) => (
        <div className="flex justify-start">
          <button
            ...
            className="text-sky-700 hover:text-sky-950 transition-colors p-2 hover:bg-gray-800 rounded-lg"
          >
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Tenant Profile Card */}
        <div className="bg-white rounded-2xl p-6 relative shadow-[0_4px_20px_rgba(14,165,233,0.08)] border border-sky-100">
          <div className="absolute top-6 right-6">
            <span
              className={`px-4 py-1 text-xs font-bold rounded-full uppercase tracking-wider ${statusLower === "active" ? "bg-[#4285F4]/20 text-[#4285F4]" : "bg-gray-800 text-sky-700"}`}
            >
          ...
          {tenant.image || tenant.profile_picture ? (
            <img
              ...
              alt={tenant.name}
              className="w-16 h-16 rounded-2xl object-cover mb-4 shadow-md border border-sky-100"
            />
          ) : (
            <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-sky-400 to-sky-600 flex items-center justify-center text-white text-2xl font-bold mb-4 shadow-md">
              {tenant.name ? tenant.name.charAt(0).toUpperCase() : "T"}
            </div>
          )}

          <h2 className="text-sky-950 text-2xl font-bold mb-1">
            {tenant.name || "Unknown Name"}
          </h2>
          <p className="text-sky-700 text-sm mb-6">Tenant Profile</p>
        ...
        {/* Usage Overview Card */}
        <div className="bg-white rounded-2xl p-6 shadow-[0_4px_20px_rgba(14,165,233,0.08)] border border-sky-100 flex flex-col">
          <div className="flex justify-between items-start mb-2">
            <div>
              <h2 className="text-sky-950 text-lg font-bold">Usage Overview</h2>
            ...
            <div className="w-[140px]">
              <Dropdown
                ...
                inputClass="!bg-white !border !border-sky-200 !text-sky-950 !rounded-lg !py-2 !px-3 !font-medium !text-sm !cursor-pointer hover:!border-sky-300 transition-colors shadow-sm"
                optionClass="!bg-white !text-sky-950 !border !border-sky-100 !rounded-lg !shadow-xl !mt-1"
                icon="!text-sky-700 !right-2"
              />
            </div>
          </div>

          <div className="flex-1 bg-sky-50/50 rounded-xl border border-sky-100 mt-4 relative flex items-center justify-center py-10 min-h-[280px]">
            <div className="flex w-full items-center justify-center gap-2 sm:gap-6 px-4">
              <div className="text-xs sm:text-sm font-medium text-sky-800 text-right flex-1">
              ...
              <div className="text-xs sm:text-sm font-medium text-sky-800 text-left flex-1">
      ...
      {/* Tabs Section */}
      <div className="bg-white shadow-[0_4px_20px_rgba(14,165,233,0.08)] rounded-2xl border border-sky-100 overflow-hidden w-full mt-8">
        <div className="flex border-b border-sky-100 bg-sky-50/30 px-2 pt-2">
          <button
            onClick={() => setActiveTab("agents")}
            className={`px-6 py-4 text-sm font-medium transition-colors ${activeTab === "agents" ? "text-sky-950 border-b-2 border-[#4285F4]" : "text-sky-700 hover:text-gray-900"}`}
          >
            AI Agents
          </button>
          <button
            onClick={() => setActiveTab("calls")}
            className={`px-6 py-4 text-sm font-medium transition-colors ${activeTab === "calls" ? "text-sky-950 border-b-2 border-[#4285F4]" : "text-sky-700 hover:text-gray-900"}`}
          >
            Calls
          </button>
          <button
            onClick={() => setActiveTab("orders")}
            className={`px-6 py-4 text-sm font-medium transition-colors ${activeTab === "orders" ? "text-sky-950 border-b-2 border-[#4285F4]" : "text-sky-700 hover:text-gray-900"}`}
          >
            Orders
          </button>
          <button
            onClick={() => setActiveTab("billing")}
            className={`px-6 py-4 text-sm font-medium transition-colors ${activeTab === "billing" ? "text-sky-950 border-b-2 border-[#4285F4]" : "text-sky-700 hover:text-gray-900"}`}
          >
            Billing History
          </button>
        </div>
        ...
        <div className="p-4">
          {activeTab === "agents" && (
            <Table
              ...
              emptyState={
                <div className="text-center py-12 text-sky-600 flex flex-col items-center justify-center gap-3">
                  <Icon icon="lucide:bot" className="text-4xl text-sky-500" />
                    ...
                    .map((line, idx) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <div
                            className={`bg-[#1A1A1A] text-gray-900 px-5 py-3.5 rounded-2xl text-[15px] max-w-[80%] ${isUser ? "rounded-tr-sm" : "rounded-tl-sm"}`}
                          >
                            {content}
                          </div>
                        </div>
