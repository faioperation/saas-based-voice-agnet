      ...
      <div className="mt-8 bg-white shadow-[0_4px_20px_rgba(14,165,233,0.08)] p-6 rounded-2xl border border-sky-100 relative">
        ...
        <div className="flex flex-col space-y-3" ref={dropdownRef}>
          {displayAgents.length > 0 ? displayAgents.map((agent) => (
            <div 
              key={agent.id} 
              className="bg-white rounded-xl p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between border border-sky-200 gap-4 sm:gap-0"
            >
              <div className="flex items-center space-x-4 w-full sm:w-auto">
                <div className="w-12 h-12 bg-sky-100 rounded-lg flex items-center justify-center shrink-0">
                  <FileText className="w-6 h-6 text-sky-700" strokeWidth={1.5} />
                </div>
                <div className="min-w-0 flex-1">
                  <h3 className="text-sky-950 font-medium text-base mb-1 truncate">{agent.name || 'Unnamed Agent'}</h3>
              ...
              <div className="flex items-center justify-between w-full sm:w-auto pt-3 sm:pt-0 border-t border-sky-200 sm:border-0">
                <div className="flex items-center gap-4 sm:gap-8">
                  <div className="flex items-center">
                    <div className={`w-2 h-2 rounded-full mr-2 shrink-0 ${agent.status === 'active' ? 'bg-green-500' : 'bg-yellow-500'}`}></div>
                    <span className="text-sky-950 text-sm font-medium capitalize">{agent.status || 'Unknown'}</span>
                  </div>
                  <div className="text-sky-700 text-sm sm:w-24 sm:text-right">
                ...
                <div className="relative ml-2 sm:ml-6">
                  <button 
                    ...
                    className="text-sky-700 hover:text-sky-950 transition-colors shrink-0 cursor-pointer p-1 rounded-md hover:bg-white/5"
                  >
            ...
            <div className="text-center text-sky-700 py-6 text-sm">No agents created yet.</div>
          )}
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-white/60 backdrop-blur-sm px-4">
          <div className="bg-white border border-sky-200 p-6 rounded-2xl shadow-2xl max-w-sm w-full animate-in fade-in zoom-in duration-200">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h3 className="text-xl font-semibold text-sky-950">Delete Agent</h3>
            </div>
            
            <p className="text-sky-700 text-sm mb-6 leading-relaxed">
              Are you sure you want to delete <span className="text-sky-950 font-medium">"{agentToDelete.name || 'Unnamed Agent'}"</span>? This action cannot be undone and the agent will be permanently removed.
            </p>
            ...
            <div className="flex justify-end gap-3">
              ...
              <button 
                ...
                disabled={deleteAgentMutation.isPending}
                className="px-5 py-2.5 text-sm font-medium text-sky-950 bg-red-600 hover:bg-red-700 rounded-xl transition-colors flex items-center gap-2 cursor-pointer disabled:opacity-50 shadow-[0_0_15px_rgba(220,38,38,0.3)]"
              >
