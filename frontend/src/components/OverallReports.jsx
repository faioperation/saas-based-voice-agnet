    ...
    <div className="w-full h-full bg-white shadow-[0_4px_20px_rgba(14,165,233,0.08)] rounded-2xl p-6 border border-sky-100 flex flex-col">
      ...
      <div className="relative flex justify-center items-center h-[260px] mb-8">
        <ResponsiveContainer width="100%" height="100%">
          <PieChart>
            ...
            <Tooltip 
              ...
              contentStyle={{ backgroundColor: '#ffffff', borderColor: '#e0f2fe', borderRadius: '12px', color: '#0c4a6e', boxShadow: '0 10px 15px -3px rgba(14,165,233,0.1)' }}
              itemStyle={{ color: '#0c4a6e', fontWeight: 500 }}
            />
          </PieChart>
        </ResponsiveContainer>
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
          <span className="text-sky-950 text-sm font-medium">Over all</span>
          <span className="text-sky-950 text-4xl font-bold mt-1">{percentage}%</span>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-5 mt-auto pb-2">
        {legendData.map((item, index) => (
          <div key={index} className="flex items-center gap-4">
            ...
            />
            <span className="text-sky-950 text-[15px] font-medium">{item.name}</span>
          </div>
        ))}
      </div>
    </div>
